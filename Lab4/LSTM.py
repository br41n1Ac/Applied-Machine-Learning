...
from keras.layers import Flatten, SimpleRNN, Dropout, Bidirectional, LSTM
...
checkpoint = ModelCheckpoint("best_model.hdf3", monitor='loss', verbose=1,
...
try:
    model = pickle.load(open("model11lstmb10.p", "rb"))
except FileNotFoundError:
    model.fit(X_train, y_train, epochs=EPOCHS, batch_size=BATCH_SIZE, validation_data=(X_val, y_val),
              callbacks=[checkpoint])
    pickle.dump(model, open("model11lstmb10.p", "wb"))
...
for i in range(len(x_test1)):
    if x_test1[i] != 0:
        ...
        prd.append(pred1[i])
write_to_file('outLSTM', xtest, prd, y_test2, rev_ner, rev_word)
